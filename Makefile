...
#	INC => '-I/opt/Minerva/include -I/opt/gnu/lib/perl5/i486-linux/DBI'
#	LIBS => ['-L/opt/Minerva/lib -lmsql -lm']
#	NAME => 'DBD::mSQL'
#	OBJECT => '$(O_FILES)'
#	VERSION => '0.60pl9'
#	dynamic_lib => { OTHERLDFLAGS=>'-L$(MSQL_HOME)/lib -L/opt/Minerva/lib' }
...
# These definitions are from config.sh (via /opt/gnu/lib/perl5/i486-linux/Config.pm)
CC = cc
LIBC = /usr/lib/libc.a
...
INST_LIB = ./blib
INST_ARCHLIB = ./blib/i486-linux
INST_EXE = ./blib/i486-linux
...
PERL_INC = /opt/gnu/lib/perl5/i486-linux/CORE
# Perl binaries
PERL = /usr/bin/perl
...
#
EXTRALIBS  = -L/opt/Minerva/lib -lmsql
LDLOADLIBS = -L/opt/Minerva/lib -lmsql -lm
BSLOADLIBS = 
LD_RUN_PATH= /opt/Minerva/lib:-lmsql:-lm


# --- MakeMaker const_cccmd section:
CCCMD = $(CC) -c -D__USE_BSD_SIGNAL -Dbool=char -DHAS_BOOL -O2 $(DEFINE_VERSION)
...
ARMAYBE = :
OTHERLDFLAGS = -L$(MSQL_HOME)/lib -L/opt/Minerva/lib
...
# installpm: mSQL.pm => $(INST_LIBDIR)/mSQL.pm, splitlib=$(INST_LIB)

$(INST_LIBDIR)/mSQL.pm: mSQL.pm Makefile $(INST_LIBDIR)/.exists
	@ rm -f $@
	$(UMASK_NULL) && cp mSQL.pm $@
	@ echo $@ >> $(INST_ARCHAUTODIR)/.packlist
...
clean ::
	-rm -rf mSQL.c ./blib Makeaperlfile $(INST_ARCHAUTODIR)/extralibs.all perlmain.c mon.out core so_locations *~ */*~ */*/*~ *.$(OBJ_EXT) *.$(LIB_EXT) perl.exe $(BOOTSTRAP) $(BASEEXT).bso $(BASEEXT).def $(BASEEXT).exp
